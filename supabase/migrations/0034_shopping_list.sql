-- ============================================================================
--  PROJECT 90 — Lista de compras (mercado)
--
--  Lista simples e persistente (não é por dia, tipo os itens de refeição em
--  Treino): você vai adicionando o que precisa comprar, marca conforme
--  compra, e limpa os marcados depois da ida ao mercado.
--
--    shopping_items — user_id, id, label, bought, created_at
--
--  Aplicar depois de 0033_daily_priorities.sql.
--  reset_progress() continua NÃO tocando nisso — é uma lista pessoal,
--  não um hábito do desafio (mesmo tratamento do diário/tarefas).
-- ============================================================================
...
drop policy if exists "shopping_items: select own" on public.shopping_items;
create policy "shopping_items: select own" on public.shopping_items for select using (auth.uid() = user_id);
drop policy if exists "shopping_items: insert own" on public.shopping_items;
create policy "shopping_items: insert own" on public.shopping_items for insert with check (auth.uid() = user_id);
drop policy if exists "shopping_items: update own" on public.shopping_items;
create policy "shopping_items: update own" on public.shopping_items for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "shopping_items: delete own" on public.shopping_items;
create policy "shopping_items: delete own" on public.shopping_items for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0033 + shoppingItems
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0033 (não toca na lista)
-- ============================================================================
