-- ============================================================================
--  PROJECT 90 — Treino (musculação A/B/C) + Alimentação
--
--  Uma página só, duas seções:
--
--    TREINO — três planos (A, B, C). Cada plano é uma lista de exercícios
--    (nome + séries + faixa de reps). O treino do dia roda em rotação
--    A → B → C → A; a cada sessão você marca os exercícios e anota a carga
--    (kg) — o app mostra a última carga pra dar a progressão. Concluir a
--    sessão marca o hábito fixo "Exercitar-se" (como diário → Escrever).
--
--    ALIMENTAÇÃO — checklist de refeições fixas (editável), foco hipertrofia.
--    Zera todo dia. Sem contagem de macros.
--
--      training_exercises  — os planos A/B/C (id gerado pelo cliente)
--      training_days       — 1 linha por dia treinado: split + entries jsonb
--                            { "<exId>": { "done": bool, "kg": number } }
--      meal_items          — a lista de refeições (id gerado pelo cliente)
--      meal_days           — 1 linha por dia: ids marcados
--
--  Aplicar depois de 0023_faculdade_kanban.sql.
...
create index if not exists training_exercises_user_idx on public.training_exercises (user_id);

alter table public.training_exercises drop constraint if exists training_exercises_split_chk;
alter table public.training_exercises
  add constraint training_exercises_split_chk check (split in ('A', 'B', 'C'));
...
create index if not exists training_days_user_idx on public.training_days (user_id);
...
create index if not exists meal_days_user_idx on public.meal_days (user_id);
...
drop policy if exists "training_exercises: select own" on public.training_exercises;
create policy "training_exercises: select own" on public.training_exercises
  for select using (auth.uid() = user_id);
drop policy if exists "training_exercises: insert own" on public.training_exercises;
create policy "training_exercises: insert own" on public.training_exercises
  for insert with check (auth.uid() = user_id);
drop policy if exists "training_exercises: update own" on public.training_exercises;
create policy "training_exercises: update own" on public.training_exercises
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "training_exercises: delete own" on public.training_exercises;
create policy "training_exercises: delete own" on public.training_exercises
  for delete using (auth.uid() = user_id);

drop policy if exists "training_days: select own" on public.training_days;
create policy "training_days: select own" on public.training_days
  for select using (auth.uid() = user_id);
drop policy if exists "training_days: insert own" on public.training_days;
create policy "training_days: insert own" on public.training_days
  for insert with check (auth.uid() = user_id);
drop policy if exists "training_days: update own" on public.training_days;
create policy "training_days: update own" on public.training_days
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "training_days: delete own" on public.training_days;
create policy "training_days: delete own" on public.training_days
  for delete using (auth.uid() = user_id);

drop policy if exists "meal_items: select own" on public.meal_items;
create policy "meal_items: select own" on public.meal_items
  for select using (auth.uid() = user_id);
drop policy if exists "meal_items: insert own" on public.meal_items;
create policy "meal_items: insert own" on public.meal_items
  for insert with check (auth.uid() = user_id);
drop policy if exists "meal_items: update own" on public.meal_items;
create policy "meal_items: update own" on public.meal_items
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "meal_items: delete own" on public.meal_items;
create policy "meal_items: delete own" on public.meal_items
  for delete using (auth.uid() = user_id);

drop policy if exists "meal_days: select own" on public.meal_days;
create policy "meal_days: select own" on public.meal_days
  for select using (auth.uid() = user_id);
drop policy if exists "meal_days: insert own" on public.meal_days;
create policy "meal_days: insert own" on public.meal_days
  for insert with check (auth.uid() = user_id);
drop policy if exists "meal_days: update own" on public.meal_days;
create policy "meal_days: update own" on public.meal_days
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "meal_days: delete own" on public.meal_days;
create policy "meal_days: delete own" on public.meal_days
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também trainingExercises / trainingDays /
--  mealItems / mealDays
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — apaga também treino e alimentação
-- ============================================================================
