-- ============================================================================
--  PROJECT 90 — Biblioteca: registro de leitura por dia (hábito "Ler")
--
--  O hábito fixo "Ler" (core_key = ler) passa a ser marcado automaticamente
--  em todo dia com leitura registrada na Biblioteca — progresso avançado,
--  livro concluído ou o botão "Li hoje". Como lib_books não guarda em que
--  dia a leitura aconteceu, cada dia com leitura vira uma linha aqui.
--
--    lib_reading_days(user_id, day_num) — 1 linha por dia do desafio.
--
--  Não é tocada por reset_progress(), igual ao resto da Biblioteca.
--  Aplicar depois de 0041_lib_book_percent.sql.
-- ============================================================================

create table if not exists public.lib_reading_days (
...
alter table public.lib_reading_days enable row level security;

drop policy if exists "lib_reading_days: select own" on public.lib_reading_days;
create policy "lib_reading_days: select own" on public.lib_reading_days
  for select using (auth.uid() = user_id);
drop policy if exists "lib_reading_days: insert own" on public.lib_reading_days;
create policy "lib_reading_days: insert own" on public.lib_reading_days
  for insert with check (auth.uid() = user_id);
drop policy if exists "lib_reading_days: update own" on public.lib_reading_days;
create policy "lib_reading_days: update own" on public.lib_reading_days
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "lib_reading_days: delete own" on public.lib_reading_days;
create policy "lib_reading_days: delete own" on public.lib_reading_days
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'libReadDays'
-- ============================================================================
