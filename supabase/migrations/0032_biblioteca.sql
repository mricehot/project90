-- ============================================================================
--  PROJECT 90 — Biblioteca: registro de livros lidos
--
--  Log pessoal de leitura: título, autor, gênero, status (quero ler / lendo /
--  lido), nota, progresso de páginas, resenha/anotações, e uma meta anual de
--  quantidade de livros.
--
--    lib_books — user_id, id, title, author, genre, cover_url, status, rating,
--                current_page, total_pages, notes, started_on, finished_on
--    lib_goals — user_id, year, target  (meta de livros lidos no ano)
--
--  Aplicar depois de 0031_fin_holdings.sql.
--  reset_progress() continua NÃO tocando na biblioteca (mesmo tratamento do
--  financeiro: é um registro pessoal, não parte do desafio de 90 dias).
-- ============================================================================
...
drop policy if exists "lib_books: select own" on public.lib_books;
create policy "lib_books: select own" on public.lib_books for select using (auth.uid() = user_id);
drop policy if exists "lib_books: insert own" on public.lib_books;
create policy "lib_books: insert own" on public.lib_books for insert with check (auth.uid() = user_id);
drop policy if exists "lib_books: update own" on public.lib_books;
create policy "lib_books: update own" on public.lib_books for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "lib_books: delete own" on public.lib_books;
create policy "lib_books: delete own" on public.lib_books for delete using (auth.uid() = user_id);

create table if not exists public.lib_goals (
  user_id uuid    not null references auth.users (id) on delete cascade,
...
alter table public.lib_goals enable row level security;
drop policy if exists "lib_goals: select own" on public.lib_goals;
create policy "lib_goals: select own" on public.lib_goals for select using (auth.uid() = user_id);
drop policy if exists "lib_goals: insert own" on public.lib_goals;
create policy "lib_goals: insert own" on public.lib_goals for insert with check (auth.uid() = user_id);
drop policy if exists "lib_goals: update own" on public.lib_goals;
create policy "lib_goals: update own" on public.lib_goals for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "lib_goals: delete own" on public.lib_goals;
create policy "lib_goals: delete own" on public.lib_goals for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0031 + library.books / library.goals
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0031 (não toca na biblioteca)
-- ============================================================================
