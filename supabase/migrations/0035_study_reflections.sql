-- ============================================================================
--  PROJECT 90 — "Reflexões" (Conhecimento): filosofia, psicologia e
--  meditação sobre a vida
--
--  Um tema por dia (escolhido no client, deterministicamente, a partir de
--  js/study-topics.js — 1000 combinações de tema × perspectiva), com um
--  pequeno formulário de perguntas fixas + reflexão livre. Guardado por dia,
--  igual ao diário/prioridades do dia.
--
--    study_reflections — user_id, day_num, topic_idx, q1, q2, q3, free, updated_at
--
--  Aplicar depois de 0034_shopping_list.sql.
--  reset_progress() continua NÃO tocando nisso — é conteúdo pessoal reflexivo,
--  não um hábito do desafio (mesmo tratamento do diário/prioridades do dia).
-- ============================================================================
...
drop policy if exists "study_reflections: select own" on public.study_reflections;
create policy "study_reflections: select own" on public.study_reflections for select using (auth.uid() = user_id);
drop policy if exists "study_reflections: insert own" on public.study_reflections;
create policy "study_reflections: insert own" on public.study_reflections for insert with check (auth.uid() = user_id);
drop policy if exists "study_reflections: update own" on public.study_reflections;
create policy "study_reflections: update own" on public.study_reflections for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "study_reflections: delete own" on public.study_reflections;
create policy "study_reflections: delete own" on public.study_reflections for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0034 + studyReflections
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0034 (não toca nas reflexões)
-- ============================================================================
