-- ============================================================================
--  PROJECT 90 — Trabalho: separa "realces" (projetos de levantamento com
--  furos, portados do BlastHole Manager) de "atividades avulsas" (entregas
--  de trabalho comuns, sem furo).
--
--  Motivo: misturar os dois tipos no mesmo card confundia — todo task
--  ganhava o botão "⛏ Furos" mesmo quando não fazia sentido (atividade
--  avulsa), e a seção de Trabalho em Pendências só olhava prazo (dueOn),
--  então realces (que não têm prazo, são acompanhados pelo progresso dos
--  furos) nunca apareciam como pendência lá.
--
--    work_projects.kind — 'realce' | 'avulsa', default 'avulsa'.
--  Backfill: projeto que já tem algum furo cadastrado em alguma tarefa
--  vira 'realce' automaticamente (só esse sinal existia antes desta coluna).
--
--  Aplicar depois de 0039_work_reminders.sql.
-- ============================================================================

alter table public.work_projects
  add column if not exists kind text not null default 'avulsa';
alter table public.work_projects
  drop constraint if exists work_projects_kind_check;
alter table public.work_projects
  add constraint work_projects_kind_check check (kind in ('realce', 'avulsa'));
...
where kind = 'avulsa'
  and exists (
    ...
    join public.work_task_holes h on h.task_id = t.id
    where t.project_id = p.id
  );

-- ============================================================================
--  app_bootstrap() — workProjects passa a incluir 'kind'
-- ============================================================================
