-- ============================================================================
--  PROJECT 90 — Faculdade: kanban + link + observações
--
--  study_activities ganha:
--    status  — 'a_fazer' | 'fazendo' | 'entregue'  (colunas do kanban)
--    link    — URL da atividade no Studeo
--    notes   — observações livres
--
--  `done` continua existindo e espelha `status = 'entregue'` (o cliente mantém
--  os dois em sincronia). "atrasada" segue derivado no cliente.
--
--  Aplicar depois de 0022_faculdade.sql.
-- ============================================================================

alter table public.study_activities
  add column if not exists status text not null default 'a_fazer',
  add column if not exists link   text,
  add column if not exists notes  text;

-- linhas antigas: quem já estava entregue vira status 'entregue'
update public.study_activities set status = 'entregue' where done is true and status <> 'entregue';

alter table public.study_activities drop constraint if exists study_activities_status_chk;
alter table public.study_activities
  add constraint study_activities_status_chk check (status in ('a_fazer', 'fazendo', 'entregue'));

-- ============================================================================
--  app_bootstrap() — studyActivities passa a devolver status / link / notes
-- ============================================================================
