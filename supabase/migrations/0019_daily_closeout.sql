-- ============================================================================
--  PROJECT 90 — Fechamento do dia (ritual noturno)
--
--  1 linha por dia do desafio em que o usuário rodou o ritual de "fechar o
--  dia": revisar hábitos, deixar uma linha no diário, ver o vocabulário
--  pendente e definir UMA prioridade para o dia seguinte.
--
--    closed_at         — quando fechou (mostrado como "fechado às 22:14")
--    tomorrow_priority  — a única prioridade escolhida para amanhã (texto livre)
--    priority_done      — marcada como feita no dia seguinte
--
--  Aplicar depois de 0018_streak_counters.sql.
-- ============================================================================
...
create table if not exists public.daily_closeouts (
  ...
  priority_done     boolean     not null default false,
  primary key (user_id, day_num)
);
create index if not exists daily_closeouts_user_idx on public.daily_closeouts (user_id);

alter table public.daily_closeouts enable row level security;

drop policy if exists "daily_closeouts: select own" on public.daily_closeouts;
create policy "daily_closeouts: select own" on public.daily_closeouts
  for select using (auth.uid() = user_id);
drop policy if exists "daily_closeouts: insert own" on public.daily_closeouts;
create policy "daily_closeouts: insert own" on public.daily_closeouts
  for insert with check (auth.uid() = user_id);
drop policy if exists "daily_closeouts: update own" on public.daily_closeouts;
create policy "daily_closeouts: update own" on public.daily_closeouts
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "daily_closeouts: delete own" on public.daily_closeouts;
create policy "daily_closeouts: delete own" on public.daily_closeouts
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'closeouts'
-- ============================================================================
...
as $$
  select jsonb_build_object(
    ...
    'closeouts', coalesce((
      select jsonb_object_agg(dc.day_num::text,
               jsonb_build_object('closedAt', dc.closed_at,
                                  'tomorrowPriority', dc.tomorrow_priority,
                                  'priorityDone', dc.priority_done))
      from public.daily_closeouts dc where dc.user_id = auth.uid()
    ), '{}'::jsonb)
  );
$$;

-- ============================================================================
--  reset_progress() — também apaga os fechamentos de dia
-- ============================================================================
...
begin
  ...
  delete from public.daily_closeouts       where user_id = auth.uid();
