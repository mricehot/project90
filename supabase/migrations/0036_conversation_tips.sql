-- ============================================================================
--  PROJECT 90 — "Conversação" (Dicção): dicas de comunicação interpessoal
--
--  Biblioteca curada de dicas (js/conversation-tips.js — conteúdo estático,
--  igual study-topics.js/bible-plan.js), organizada por categoria. Só o
--  ESTADO "lida" de cada dica é persistido — não o texto, que já vive no
--  client — igual bible_days marca dias do plano da Bíblia sem duplicar o
--  texto do capítulo.
--
--    conversation_tips_read — user_id, tip_idx, read_day
--
--  Aplicar depois de 0035_study_reflections.sql.
--  reset_progress() continua NÃO tocando nisso — é progresso de leitura
--  pessoal, não um hábito do desafio (mesmo tratamento das leituras da
--  Bíblia/textos de leitura em voz alta).
-- ============================================================================

create table if not exists public.conversation_tips_read (
  user_id  uuid not null references auth.users (id) on delete cascade,
  tip_idx  int  not null,
  read_day int  not null,
  primary key (user_id, tip_idx)
);

alter table public.conversation_tips_read enable row level security;
drop policy if exists "conversation_tips_read: select own" on public.conversation_tips_read;
create policy "conversation_tips_read: select own" on public.conversation_tips_read for select using (auth.uid() = user_id);
drop policy if exists "conversation_tips_read: insert own" on public.conversation_tips_read;
create policy "conversation_tips_read: insert own" on public.conversation_tips_read for insert with check (auth.uid() = user_id);
drop policy if exists "conversation_tips_read: delete own" on public.conversation_tips_read;
create policy "conversation_tips_read: delete own" on public.conversation_tips_read for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0035 + conversationTipsRead
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0035 (não toca nas dicas lidas)
-- ============================================================================
