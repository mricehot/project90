-- ============================================================================
--  PROJECT 90 — Biblioteca: progresso por porcentagem (Kindle)
--
--  Kindle não mostra número de página, só porcentagem lida. Até aqui o
--  progresso de um livro só existia como página atual / total de páginas —
--  impossível de preencher direito pra quem lê só no Kindle. Agora cada
--  livro escolhe o próprio jeito de acompanhar progresso.
--
--    lib_books.progress_mode — 'pages' | 'percent', default 'pages'.
--    lib_books.progress_pct  — 0-100, só usado quando progress_mode='percent'.
--
--  Aplicar depois de 0040_work_project_kind.sql.
-- ============================================================================

alter table public.lib_books
  add column if not exists progress_mode text not null default 'pages';
alter table public.lib_books
  drop constraint if exists lib_books_progress_mode_check;
alter table public.lib_books
  add constraint lib_books_progress_mode_check check (progress_mode in ('pages', 'percent'));

alter table public.lib_books
  add column if not exists progress_pct integer;
alter table public.lib_books
  drop constraint if exists lib_books_progress_pct_check;
alter table public.lib_books
  add constraint lib_books_progress_pct_check check (progress_pct is null or (progress_pct >= 0 and progress_pct <= 100));

-- ============================================================================
--  app_bootstrap() — library.books passa a incluir 'progressMode'/'progressPct'
-- ============================================================================
