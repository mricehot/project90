-- ============================================================================
--  PROJECT 90 — Pendências › "Ao chegar em casa"
--
--  Checklist do que fazer assim que chega em casa (mesmo espírito da Rotina
--  noturna, mas uma única lista). Zera todo dia: os itens ficam e as marcações
--  são por dia do desafio.
--
--    home_habits(user_id, id, label, created_at)         — a lista
--    home_routine_days(user_id, day_num, done_ids, ...)  — o que foi marcado em cada dia
--
--  Não é tocada por reset_progress(). Aplicar depois de 0049_reset_keeps_habits.sql.
-- ============================================================================

create table if not exists public.home_habits (
...
create index if not exists home_habits_user_idx on public.home_habits (user_id);

create table if not exists public.home_routine_days (
  user_id    uuid        not null references auth.users (id) on delete cascade,
...
create index if not exists home_routine_days_user_idx on public.home_routine_days (user_id);

alter table public.home_habits       enable row level security;
alter table public.home_routine_days enable row level security;

drop policy if exists "home_habits: select own" on public.home_habits;
create policy "home_habits: select own" on public.home_habits
  for select using (auth.uid() = user_id);
drop policy if exists "home_habits: insert own" on public.home_habits;
create policy "home_habits: insert own" on public.home_habits
  for insert with check (auth.uid() = user_id);
drop policy if exists "home_habits: update own" on public.home_habits;
create policy "home_habits: update own" on public.home_habits
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "home_habits: delete own" on public.home_habits;
create policy "home_habits: delete own" on public.home_habits
  for delete using (auth.uid() = user_id);

drop policy if exists "home_routine_days: select own" on public.home_routine_days;
create policy "home_routine_days: select own" on public.home_routine_days
  for select using (auth.uid() = user_id);
drop policy if exists "home_routine_days: insert own" on public.home_routine_days;
create policy "home_routine_days: insert own" on public.home_routine_days
  for insert with check (auth.uid() = user_id);
drop policy if exists "home_routine_days: update own" on public.home_routine_days;
create policy "home_routine_days: update own" on public.home_routine_days
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "home_routine_days: delete own" on public.home_routine_days;
create policy "home_routine_days: delete own" on public.home_routine_days
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'homeHabits' e 'homeRoutineDays'
-- ============================================================================
...
as $$
  select jsonb_build_object(
    ...
    'finance', jsonb_build_object(
      ...
      'holdings', coalesce((
        select jsonb_agg(jsonb_build_object('ticker', hd.ticker, 'cotas', hd.cotas, 'price', hd.price,
                                            'priceUpdatedAt', hd.price_updated_at)
                 order by hd.ticker)
        from public.fin_holdings hd where hd.user_id = auth.uid()
      ), '[]'::jsonb)
    ),
    'library', jsonb_build_object(
      'books', coalesce((
        select jsonb_agg(jsonb_build_object(
                 ...
                 'status', b.status, 'rating', b.rating, 'currentPage', b.current_page,
                 'totalPages', b.total_pages,
                 'progressMode', coalesce(b.progress_mode, 'pages'), 'progressPct', b.progress_pct,
                 'notes', b.notes,
                 'startedOn', b.started_on, 'finishedOn', b.finished_on, 'createdAt', b.created_at)
                 order by b.created_at desc, b.id desc)
        from public.lib_books b where b.user_id = auth.uid()
      ), '[]'::jsonb),
      'goals', coalesce((
        select jsonb_object_agg(g.year::text, g.target)
        from public.lib_goals g where g.user_id = auth.uid()
      ), '{}'::jsonb)
    ),
    'dailyPriorities', coalesce((
      select jsonb_object_agg(dp.day_num::text, dp.items)
      from public.daily_priorities dp where dp.user_id = auth.uid()
    ), '{}'::jsonb),
    'shoppingItems', coalesce((
      select jsonb_agg(jsonb_build_object('id', si.id, 'label', si.label, 'bought', si.bought,
                                          'createdAt', si.created_at)
               order by si.bought, si.id)
      from public.shopping_items si where si.user_id = auth.uid()
    ), '[]'::jsonb),
    'studyReflections', coalesce((
      select jsonb_object_agg(r.day_num::text,
               jsonb_build_object('topicIdx', r.topic_idx, 'q1', r.q1, 'q2', r.q2, 'q3', r.q3, 'free', r.free))
      from public.study_reflections r where r.user_id = auth.uid()
    ), '{}'::jsonb),
    'conversationTipsRead', coalesce((
      select jsonb_object_agg(c.tip_idx::text, c.read_day)
      from public.conversation_tips_read c where c.user_id = auth.uid()
    ), '{}'::jsonb),
    'workProjects', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', wp.id, 'name', wp.name,
                                  'archived', wp.archived, 'kind', coalesce(wp.kind, 'avulsa'),
                                  'createdAt', wp.created_at)
               order by wp.sort_order, wp.id
             )
      from public.work_projects wp where wp.user_id = auth.uid()
    ), '[]'::jsonb),
    'workTasks', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', wt.id, 'projectId', wt.project_id, 'title', wt.title,
                                  'dueOn', wt.due_on, 'status', coalesce(wt.status, 'a_fazer'),
                                  'done', wt.done, 'doneOn', wt.done_on,
                                  'link', wt.link, 'notes', wt.notes,
                                  'createdAt', wt.created_at)
               order by wt.due_on nulls last, wt.id
             )
      from public.work_tasks wt where wt.user_id = auth.uid()
    ), '[]'::jsonb),
    'workTaskHoles', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', wh.id, 'taskId', wh.task_id, 'number', wh.number,
                                  'profiled', wh.profiled, 'surveyed', wh.surveyed, 'meters', wh.meters,
                                  'profiledAt', wh.profiled_at, 'surveyedAt', wh.surveyed_at,
                                  'createdAt', wh.created_at)
               order by wh.task_id, wh.number
             )
      from public.work_task_holes wh where wh.user_id = auth.uid()
    ), '[]'::jsonb),
    'workReminders', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', wr.id, 'text', wr.text, 'taskId', wr.task_id,
                                  'createdAt', wr.created_at)
               order by wr.created_at, wr.id
             )
      from public.work_reminders wr where wr.user_id = auth.uid()
    ), '[]'::jsonb),
    'libReadDays', coalesce((
      select jsonb_object_agg(lr.day_num::text, true)
      from public.lib_reading_days lr where lr.user_id = auth.uid()
    ), '{}'::jsonb),
    'waterDays', coalesce((
      select jsonb_object_agg(wd.day_num::text, wd.ml)
      from public.water_days wd where wd.user_id = auth.uid()
    ), '{}'::jsonb),
    'homeHabits', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', hh.id, 'label', hh.label, 'createdAt', hh.created_at)
               order by hh.created_at, hh.id
             )
      from public.home_habits hh where hh.user_id = auth.uid()
    ), '[]'::jsonb),
    'homeRoutineDays', coalesce((
      select jsonb_object_agg(hr.day_num::text, hr.done_ids)
      from public.home_routine_days hr where hr.user_id = auth.uid()
    ), '{}'::jsonb),
    'idleState', (select i.state from public.idle_state i where i.user_id = auth.uid())
  );
$$;
