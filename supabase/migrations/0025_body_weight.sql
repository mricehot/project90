-- ============================================================================
--  PROJECT 90 — Peso corporal
--
--  Um registro simples de peso pra acompanhar junto com a evolução de carga
--  na página de Treino. Uma pesagem por dia do desafio (upsert).
--
--    body_weights — user_id, day_num (dia do desafio), kg numeric
--
--  A evolução de CARGA por exercício não precisa de tabela — já vem de
--  training_days.entries[exId].kg (migração 0024).
--
--  Aplicar depois de 0024_training.sql.
...
drop policy if exists "body_weights: select own" on public.body_weights;
create policy "body_weights: select own" on public.body_weights
  for select using (auth.uid() = user_id);
drop policy if exists "body_weights: insert own" on public.body_weights;
create policy "body_weights: insert own" on public.body_weights
  for insert with check (auth.uid() = user_id);
drop policy if exists "body_weights: update own" on public.body_weights;
create policy "body_weights: update own" on public.body_weights
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "body_weights: delete own" on public.body_weights;
create policy "body_weights: delete own" on public.body_weights
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'bodyWeights'
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — apaga também o peso corporal
-- ============================================================================
