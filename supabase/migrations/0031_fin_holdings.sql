-- ============================================================================
--  PROJECT 90 — Financeiro: posições em FII (cotas × preço)
--
--  Em vez de digitar o valor total da carteira, o usuário mantém a posição
--  de cada fundo (quantidade de cotas) + o último preço conhecido da cota.
--  O patrimônio passa a ser Σ (cotas × preço), atualizado só mexendo nos
--  preços. O snapshot mensal do patrimônio é alimentado por esse cálculo.
--
--    fin_holdings — user_id, ticker, cotas, price, price_updated_at
--
--  Aplicar depois de 0030_fin_snapshots.sql.
...
create table if not exists public.fin_holdings (
  ...
  cotas            numeric(14,4) not null default 0,
...
drop policy if exists "fin_holdings: select own" on public.fin_holdings;
create policy "fin_holdings: select own" on public.fin_holdings for select using (auth.uid() = user_id);
drop policy if exists "fin_holdings: insert own" on public.fin_holdings;
create policy "fin_holdings: insert own" on public.fin_holdings for insert with check (auth.uid() = user_id);
drop policy if exists "fin_holdings: update own" on public.fin_holdings;
create policy "fin_holdings: update own" on public.fin_holdings for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "fin_holdings: delete own" on public.fin_holdings;
create policy "fin_holdings: delete own" on public.fin_holdings for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0030 + finance.holdings
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0030 (não toca no financeiro)
-- ============================================================================
