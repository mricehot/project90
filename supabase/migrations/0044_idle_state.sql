-- ============================================================================
--  PROJECT 90 — Sistema › Masmorra: estado do jogo idle
--
--  O jogo idle da aba "Masmorra" (herói que luta sozinho, energia vinda dos
--  hábitos) guarda tudo em um único blob jsonb por usuário: andar, ouro,
--  melhorias, energia gasta, progresso da semana no chefe mundial etc.
--  Nada é derivado do servidor — o cliente recalcula o progresso offline.
--
--    idle_state(user_id pk, state jsonb, updated_at)
--
--  Não é tocada por reset_progress() (jogo pessoal, independente do desafio).
--  Aplicar depois de 0043_water_days.sql.
-- ============================================================================
...
drop policy if exists "idle_state: select own" on public.idle_state;
create policy "idle_state: select own" on public.idle_state
  for select using (auth.uid() = user_id);
drop policy if exists "idle_state: insert own" on public.idle_state;
create policy "idle_state: insert own" on public.idle_state
  for insert with check (auth.uid() = user_id);
drop policy if exists "idle_state: update own" on public.idle_state;
create policy "idle_state: update own" on public.idle_state
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "idle_state: delete own" on public.idle_state;
create policy "idle_state: delete own" on public.idle_state
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'idleState'
-- ============================================================================
