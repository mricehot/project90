-- ============================================================================
--  PROJECT 90 — Trabalho (controle de projetos e entregas)
--
--  Mesmo espírito de Faculdade, só que pra vida profissional: projetos →
--  tarefas/entregas com prazo, num kanban (a_fazer/fazendo/entregue) desde o
--  início — Faculdade precisou de duas migrations (0022 + 0023) pra chegar
--  nesse formato, aqui já nasce assim.
--
--    work_projects — projetos (id gerado pelo cliente)
--    work_tasks    — tarefas/entregas: título, due_on, status, link, notes
--
--  Status "atrasada" é derivado no cliente (venceu e não entregue), igual
--  study_activities. reset_progress() NÃO mexe aqui — mesma regra já
--  aplicada a faculdade/financeiro/biblioteca: só hábitos são resetados.
--
--  Aplicar depois de 0036_conversation_tips.sql.
-- ============================================================================
...
create index if not exists work_projects_user_idx on public.work_projects (user_id);
...
create index if not exists work_tasks_user_idx    on public.work_tasks (user_id);
create index if not exists work_tasks_due_idx     on public.work_tasks (user_id, due_on);
create index if not exists work_tasks_project_idx on public.work_tasks (user_id, project_id);

alter table public.work_tasks drop constraint if exists work_tasks_status_chk;
alter table public.work_tasks
  add constraint work_tasks_status_chk check (status in ('a_fazer', 'fazendo', 'entregue'));

alter table public.work_projects enable row level security;
alter table public.work_tasks    enable row level security;

drop policy if exists "work_projects: select own" on public.work_projects;
create policy "work_projects: select own" on public.work_projects
  for select using (auth.uid() = user_id);
drop policy if exists "work_projects: insert own" on public.work_projects;
create policy "work_projects: insert own" on public.work_projects
  for insert with check (auth.uid() = user_id);
drop policy if exists "work_projects: update own" on public.work_projects;
create policy "work_projects: update own" on public.work_projects
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "work_projects: delete own" on public.work_projects;
create policy "work_projects: delete own" on public.work_projects
  for delete using (auth.uid() = user_id);

drop policy if exists "work_tasks: select own" on public.work_tasks;
create policy "work_tasks: select own" on public.work_tasks
  for select using (auth.uid() = user_id);
drop policy if exists "work_tasks: insert own" on public.work_tasks;
create policy "work_tasks: insert own" on public.work_tasks
  for insert with check (auth.uid() = user_id);
drop policy if exists "work_tasks: update own" on public.work_tasks;
create policy "work_tasks: update own" on public.work_tasks
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "work_tasks: delete own" on public.work_tasks;
create policy "work_tasks: delete own" on public.work_tasks
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'workProjects' + 'workTasks'
-- ============================================================================
    ...
    'library', jsonb_build_object(
      'books', coalesce((
        select jsonb_agg(jsonb_build_object(
                 ...
                 'totalPages', b.total_pages, 'notes', b.notes,
    ...
    'workProjects', coalesce((
      select jsonb_agg(
               jsonb_build_object('id', wp.id, 'name', wp.name,
                                  'archived', wp.archived, 'createdAt', wp.created_at)
...
-- ============================================================================
--  reset_progress() — redeclarada idêntica (0028): só hábitos são resetados.
--  Trabalho fica de fora, igual faculdade/financeiro/biblioteca.
-- ============================================================================
