-- ============================================================================
--  PROJECT 90 — Banco de provas (registro de vitórias)
--
--  Uma lista corrida de evidências de que a pessoa é capaz: "resolvi X",
--  "encarei a conversa difícil", "mantive a rotina numa semana caótica".
--  Entradas manuais; o card no dashboard também mostra as conquistas
--  desbloqueadas (derivadas de `achievements`, sem linha em `wins`).
--
--  Aplicar depois de 0016_journal_prompt.sql.
-- ============================================================================
...
create index if not exists wins_user_idx on public.wins (user_id);

alter table public.wins enable row level security;

drop policy if exists "wins: select own" on public.wins;
create policy "wins: select own" on public.wins
  for select using (auth.uid() = user_id);
drop policy if exists "wins: insert own" on public.wins;
create policy "wins: insert own" on public.wins
  for insert with check (auth.uid() = user_id);
drop policy if exists "wins: update own" on public.wins;
create policy "wins: update own" on public.wins
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "wins: delete own" on public.wins;
create policy "wins: delete own" on public.wins
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'wins'
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — também apaga o banco de provas
-- ============================================================================
