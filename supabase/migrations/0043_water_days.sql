-- ============================================================================
--  PROJECT 90 — Água: contador diário (hábito "Beber água")
--
--  Card "Água" em Pendências: botões +250/+500 ml somam o total do dia.
--  Bater a meta do hábito fixo Beber água (campo "meta", ex.: "2 L") marca
--  o hábito automaticamente naquele dia.
--
--    water_days(user_id, day_num, ml) — total em ml por dia do desafio.
--
--  Não é tocada por reset_progress() (mesmo tratamento dos logs pessoais).
--  Aplicar depois de 0042_lib_reading_days.sql.
-- ============================================================================
...
drop policy if exists "water_days: select own" on public.water_days;
create policy "water_days: select own" on public.water_days
  for select using (auth.uid() = user_id);
drop policy if exists "water_days: insert own" on public.water_days;
create policy "water_days: insert own" on public.water_days
  for insert with check (auth.uid() = user_id);
drop policy if exists "water_days: update own" on public.water_days;
create policy "water_days: update own" on public.water_days
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "water_days: delete own" on public.water_days;
create policy "water_days: delete own" on public.water_days
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'waterDays'
-- ============================================================================
