-- ============================================================================
--  PROJECT 90 — o dia vira às 5h da manhã
--
--  Quem chega tarde precisa poder fechar os hábitos do dia anterior: o "dia"
--  do desafio agora começa às 05:00 (no fuso do usuário), não à meia-noite.
--  O cliente (Store.now()) já usa o mesmo corte; aqui alinhamos o servidor:
--    · challenge_day()  — dia atual = data local de (agora − 5h)
--    · set_timezone()   — ancoragem do dia 1 com o mesmo corte
--    · reset_progress() — o novo dia 1 começa na data lógica de hoje
--  app_bootstrap() não muda (só chama challenge_day()).
--
--  Aplicar depois de 0044_idle_state.sql.
-- ============================================================================
...
as $$
  select greatest(1, least(
           (((now() at time zone coalesce(nullif(m.timezone, ''), 'UTC')) - interval '5 hours')::date - m.start_date) + 1,
           m.total_days
         ))
...
begin
  ...
  begin
    local_today := ((now() at time zone p_tz) - interval '5 hours')::date;   -- erro se o fuso for inválido
...
begin
  ...
  -- Calendário volta pro dia 1: início = hoje (dia lógico, virada às 5h),
  -- 90 dias, folgas renovadas.
  insert into public.challenge_meta (user_id, start_date, total_days, timezone, freezes_left, frozen_days)
  values (auth.uid(), ((now() at time zone tz) - interval '5 hours')::date, 90, tz, 2, '{}')
