-- ============================================================================
--  PROJECT 90 — Trabalho: lembretes
--
--  Seção de lembretes na página de Trabalho — nota visível (sem horário,
--  sem notificação agendada), opcionalmente ligada a uma tarefa. "Resolver"
--  um lembrete é exclusão definitiva (com desfazer na UI), igual ao resto
--  do app — por isso não existe coluna resolved/resolved_at aqui. O
--  desaparecimento automático quando a tarefa ligada é concluída é
--  calculado em tempo de render (comparando task_id com work_tasks.done),
--  não é um estado gravado — então reverte sozinho se a tarefa for
--  desmarcada depois.
--
--    work_reminders — id gerado pelo cliente; task_id não tem FK (mesmo
--    padrão de work_task_holes.task_id): se a tarefa for excluída, o
--    lembrete continua existindo, só sem o vínculo reconhecido no render.
--
--  Não é tocado por reset_progress(), igual o resto de Trabalho.
--
--  Aplicar depois de 0038_work_task_holes.sql.
-- ============================================================================
...
create index if not exists work_reminders_user_idx on public.work_reminders (user_id);
...
-- ============================================================================
--  app_bootstrap() — devolve também 'workReminders'
-- ============================================================================
