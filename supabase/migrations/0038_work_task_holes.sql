-- ============================================================================
--  PROJECT 90 — Trabalho: checklist de furos por tarefa
--
--  Porta o mecanismo de "leque → furos" do BlastHole Manager (projeto
--  irmão do usuário) pra dentro de uma tarefa de Trabalho: cada furo é
--  marcável em dois eixos independentes — perfilado e topografado — mais
--  uma metragem opcional. Sem "total" fixo: os furos são adicionados aos
--  poucos, por faixa (de/até), conforme o trabalho avança.
--
--    work_task_holes — furos de uma tarefa (id gerado pelo cliente)
--
--  Não é tocado por reset_progress(), igual o resto de Trabalho.
--
--  Aplicar depois de 0037_trabalho.sql.
-- ============================================================================

create table if not exists public.work_task_holes (
...
create index if not exists work_task_holes_user_idx on public.work_task_holes (user_id);
create index if not exists work_task_holes_task_idx on public.work_task_holes (user_id, task_id);

alter table public.work_task_holes enable row level security;

drop policy if exists "work_task_holes: select own" on public.work_task_holes;
create policy "work_task_holes: select own" on public.work_task_holes
  for select using (auth.uid() = user_id);
drop policy if exists "work_task_holes: insert own" on public.work_task_holes;
create policy "work_task_holes: insert own" on public.work_task_holes
  for insert with check (auth.uid() = user_id);
drop policy if exists "work_task_holes: update own" on public.work_task_holes;
create policy "work_task_holes: update own" on public.work_task_holes
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "work_task_holes: delete own" on public.work_task_holes;
create policy "work_task_holes: delete own" on public.work_task_holes
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'workTaskHoles'
-- ============================================================================
