-- ============================================================================
--  PROJECT 90 — Prioridades do dia (Top 3)
--
--  Ritual de manhã: até 3 prioridades para o dia, marcadas como feitas ao
--  longo do dia. Guardado por day_num (como journal/night_routine_days) —
--  é conteúdo pessoal do dia, não um hábito do desafio.
--
--    daily_priorities — user_id, day_num, items jsonb ([{text, done}, ...])
--
--  Aplicar depois de 0032_biblioteca.sql.
--  reset_progress() continua NÃO tocando nisso — mesmo tratamento do
--  diário/tarefas/rotina noturna: conteúdo pessoal, não parte do reset.
-- ============================================================================
...
drop policy if exists "daily_priorities: select own" on public.daily_priorities;
create policy "daily_priorities: select own" on public.daily_priorities for select using (auth.uid() = user_id);
drop policy if exists "daily_priorities: insert own" on public.daily_priorities;
create policy "daily_priorities: insert own" on public.daily_priorities for insert with check (auth.uid() = user_id);
drop policy if exists "daily_priorities: update own" on public.daily_priorities;
create policy "daily_priorities: update own" on public.daily_priorities for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "daily_priorities: delete own" on public.daily_priorities;
create policy "daily_priorities: delete own" on public.daily_priorities for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — 0032 + dailyPriorities
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0032 (não toca nas prioridades)
-- ============================================================================
