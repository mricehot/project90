-- ============================================================================
--  PROJECT 90 — Financeiro (nova página)
--
--  Controle mensal: salário + rendas extras, gastos fixos (com pago/pendente
--  por mês e vencimento), compras parceladas, gastos esporádicos, aportes e
--  dividendos de FII (+ valor da carteira digitado à mão) e objetivos.
--
--  Ciclo = mês do calendário (dia 1 ao fim). Todo lançamento pertence ao mês
--  da sua data (coluna `ym` = 'YYYY-MM').
--
--    fin_config       — 1 linha/usuário: salário base, dia do pagamento,
--                       meta de aporte, valor atual da carteira de FII
--    fin_income       — recebimentos extras do mês (o salário base vem do config)
--    fin_fixed        — molde dos gastos fixos recorrentes
--    fin_fixed_paid   — marca de "pago" de um gasto fixo num mês
--    fin_installments — compras parceladas (contribuem N meses a partir de firstYm)
--    fin_expenses     — gastos esporádicos / variáveis (o diário de gastos)
--    fin_investments  — movimentos de FII: 'aporte' | 'dividendo'
--    fin_goals        — objetivos: meta, guardado, plano mensal, prazo
--
--  Aplicar depois de 0028_reset_scope_habits.sql.
--  reset_progress() NÃO apaga o financeiro (segue o escopo "só hábitos").
...
create table if not exists public.fin_config (
  ...
  salary_day          int           not null default 5,
  invest_target       numeric(12,2) not null default 0,
...
create index if not exists fin_income_user_ym_idx on public.fin_income (user_id, ym);

create table if not exists public.fin_fixed (
  user_id    uuid          not null references auth.users (id) on delete cascade,
  id         bigint        not null,
  label      text          not null default '',
  amount     numeric(12,2) not null default 0,
...
create table if not exists public.fin_fixed_paid (
  user_id  uuid          not null references auth.users (id) on delete cascade,
  fixed_id bigint        not null,
  ym       text          not null,
  paid_on  date,
  amount   numeric(12,2) not null default 0,
  primary key (user_id, fixed_id, ym)
);
...
create index if not exists fin_expenses_user_ym_idx on public.fin_expenses (user_id, ym);

create table if not exists public.fin_investments (
  user_id    uuid          not null references auth.users (id) on delete cascade,
  id         bigint        not null,
  kind       text          not null default 'aporte',   -- 'aporte' | 'dividendo'
...
create index if not exists fin_investments_user_ym_idx on public.fin_investments (user_id, ym);
...
begin
  foreach t in array array[
    'fin_config','fin_income','fin_fixed','fin_fixed_paid',
    'fin_installments','fin_expenses','fin_investments','fin_goals'
  ]
  loop
...
-- ============================================================================
--  app_bootstrap() — 0028 + a chave 'finance'
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — inalterada em relação à 0028 (não toca no financeiro)
-- ============================================================================
