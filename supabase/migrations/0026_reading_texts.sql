-- ============================================================================
--  PROJECT 90 — Leitura em voz alta (Dicção)
--
--  A Dicção tinha só frases curtas / trava-línguas. Aqui entra uma seção
--  separada de TEXTOS mais longos pra ler em voz alta, gravar e se ouvir
--  (o gravador já existe na página — nada é salvo, só ouvir na hora).
--
--    reading_texts — user_id, id (cliente), title, body (texto longo),
--                    category ('prosa'|'poesia'|'noticia'|'dialogo'|
--                    'discurso'|'instrucao'|'outro'), source (autor/origem),
--                    last_read_day (dia do desafio da última leitura marcada)
--
--  Conjunto pronto de ~25 textos (domínio público + originais) vem do
--  cliente (js/reading-texts.js), carregado por um botão — igual à
--  biblioteca de exercícios de dicção.
--
--  Aplicar depois de 0025_body_weight.sql.
...
create table if not exists public.reading_texts (
  ...
  source        text,
  last_read_day integer,
  created_at    timestamptz not null default now(),
  primary key (user_id, id)
);
create index if not exists reading_texts_user_idx on public.reading_texts (user_id);

alter table public.reading_texts enable row level security;

drop policy if exists "reading_texts: select own" on public.reading_texts;
create policy "reading_texts: select own" on public.reading_texts
  for select using (auth.uid() = user_id);
drop policy if exists "reading_texts: insert own" on public.reading_texts;
create policy "reading_texts: insert own" on public.reading_texts
  for insert with check (auth.uid() = user_id);
drop policy if exists "reading_texts: update own" on public.reading_texts;
create policy "reading_texts: update own" on public.reading_texts
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "reading_texts: delete own" on public.reading_texts;
create policy "reading_texts: delete own" on public.reading_texts
  for delete using (auth.uid() = user_id);

-- ============================================================================
--  app_bootstrap() — devolve também 'readingTexts'
-- ============================================================================
...
-- ============================================================================
--  reset_progress() — apaga também os textos de leitura
-- ============================================================================
